...
?>
    ...
    <section class="relative min-h-[90vh] flex items-center overflow-hidden">
      ...
      <div class="relative z-10 py-20 container-custom">
        <div class="mx-auto max-w-4xl text-center">
          <?php
          $hero_badge = get_post_meta(get_the_ID(), '_sgs_hero_badge', true) ?: "Salisbury's Trusted Guttering Experts";
          $hero_title = get_post_meta(get_the_ID(), '_sgs_hero_title', true) ?: 'Professional <span class="text-secondary-400">Guttering</span> Services';
          $hero_subtitle = get_post_meta(get_the_ID(), '_sgs_hero_subtitle', true) ?: 'Expert gutter cleaning, repairs, and maintenance protecting homes and businesses across Salisbury and surrounding areas.';
          ?>
          <!-- Badge -->
          <div class="inline-flex items-center px-4 py-2 mb-6 rounded-full border backdrop-blur-sm bg-secondary-400/20 border-secondary-400/30 animate-fade-in">
            ...
            <span class="text-sm font-semibold text-secondary-400"><?php echo esc_html($hero_badge); ?></span>
          </div>
    ...
    <!-- Custom Page Content (Editable in WordPress Editor) -->
    ...
    <!-- About Us Section -->
    <?php if (get_post_meta(get_the_ID(), '_sgs_show_about', true) !== '0') :
      $about_heading = get_post_meta(get_the_ID(), '_sgs_about_heading', true) ?: 'Hi, I\'m <span class="text-secondary-400">Karl West</span>';
      $about_intro = get_post_meta(get_the_ID(), '_sgs_about_intro', true) ?: 'I\'m a sole trader with over 10 years of experience serving Salisbury and the surrounding areas. When you contact Super Guttering Services, you\'re speaking directly to me – and I\'ll be the one doing the work on your property.';
      $about_content = get_post_meta(get_the_ID(), '_sgs_about_content', true) ?: 'I understand that your gutters play a crucial role in protecting your property from water damage. That\'s why I\'m committed to providing top-quality workmanship, using the best materials, and delivering exceptional results every time.';
      $about_warning_title = get_post_meta(get_the_ID(), '_sgs_about_warning_title', true) ?: 'Beware of Fake "Local" Companies';
      $about_warning_text = get_post_meta(get_the_ID(), '_sgs_about_warning_text', true) ?: 'Many gutter companies pose as small local businesses but actually send out third-party contractors to do the work – often at inflated prices. With me, you get a genuine local tradesman who takes pride in every job. No middlemen, no surprises, just honest work at fair prices.';
      $stat_years = get_post_meta(get_the_ID(), '_sgs_stat_years', true) ?: '10+';
      $stat_customers = get_post_meta(get_the_ID(), '_sgs_stat_customers', true) ?: '500+';
      $stat_service = get_post_meta(get_the_ID(), '_sgs_stat_service', true) ?: '24/7';
    ?>
      <section class="bg-white section-padding">
        <div class="container-custom">
            ...
            <div class="fade-in-left">
              ...
              <div class="p-6 mb-8 rounded-lg border-l-4 bg-secondary-400/10 border-secondary-400 fade-in-up">
                <div class="flex items-start">
                  ...
                  <div>
                    <h4 class="mb-2 font-bold text-primary-900"><?php echo esc_html($about_warning_title); ?></h4>
                    <p class="text-sm leading-relaxed text-neutral-700">
                      <?php echo esc_html($about_warning_text); ?>
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-3 gap-6 mb-8" data-stagger="150">
                <div class="p-4 text-center rounded-xl bg-neutral-50 scale-in">
                  <div class="mb-1 text-3xl font-bold text-secondary-400"><?php echo esc_html($stat_years); ?></div>
                  <div class="text-sm text-neutral-600">Years Experience</div>
                </div>
                <div class="p-4 text-center rounded-xl bg-neutral-50 scale-in">
                  <div class="mb-1 text-3xl font-bold text-secondary-400"><?php echo esc_html($stat_customers); ?></div>
                  <div class="text-sm text-neutral-600">Happy Customers</div>
                </div>
                <div class="p-4 text-center rounded-xl bg-neutral-50">
                  <div class="mb-1 text-3xl font-bold text-secondary-400"><?php echo esc_html($stat_service); ?></div>
    ...
    <?php if (get_post_meta(get_the_ID(), '_sgs_show_video', true) !== '0') :
      $video_badge = get_post_meta(get_the_ID(), '_sgs_video_badge', true) ?: 'See Us In Action';
      $video_heading = get_post_meta(get_the_ID(), '_sgs_video_heading', true) ?: 'Serving <span class="text-secondary-400">Wiltshire</span> & Surrounding Areas';
      $video_description = get_post_meta(get_the_ID(), '_sgs_video_description', true) ?: 'Watch how we deliver professional guttering services across Wiltshire and the surrounding areas';
      $video_url = get_post_meta(get_the_ID(), '_sgs_video_url', true) ?: 'https://www.youtube.com/watch?v=VuGl4LfWVVk';

      // Extract YouTube video ID
      preg_match('/(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]+)/', $video_url, $matches);
      $video_id = $matches[1] ?? 'VuGl4LfWVVk';
    ?>
      <section class="section-padding bg-primary-900">
        <div class="container-custom">
          <div class="mb-12 text-center fade-in-up">
            <span class="inline-block px-4 py-2 mb-4 text-sm font-semibold rounded-full border bg-secondary-400/20 text-secondary-400 border-secondary-400/30"><?php echo esc_html($video_badge); ?></span>
            <h2 class="mb-4 text-4xl font-bold text-white md:text-5xl font-heading">
              <?php echo wp_kses_post($video_heading); ?>
            </h2>
            <p class="mx-auto max-w-2xl text-xl text-neutral-200">
              <?php echo esc_html($video_description); ?>
            </p>
          </div>

          <div class="mx-auto max-w-5xl fade-in-up">
            <div class="overflow-hidden relative rounded-2xl shadow-2xl aspect-video">
              <iframe
                class="absolute inset-0 w-full h-full"
                src="https://www.youtube.com/embed/<?php echo esc_attr($video_id); ?>"
                title="Super Guttering Services - Wiltshire & Surrounding Areas"
                frameborder="0"
            ...
            <div class="flex flex-col gap-4 justify-center mt-8 sm:flex-row">
              <a href="<?php echo home_url('/contact-us/'); ?>" class="inline-flex justify-center items-center px-6 py-3 font-semibold rounded-lg transition-colors bg-secondary-400 text-primary-900 hover:bg-secondary-500">
                <svg class="mr-2 w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                </svg>
                Get Your Free Quote
              </a>
              <a href="tel:07936764009" class="inline-flex justify-center items-center px-6 py-3 font-semibold text-white rounded-lg border transition-colors border-white/30 hover:bg-white/10">
                <svg class="mr-2 w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
                </svg>
                Call: 07936 764009
    ...
    <!-- Services Section -->
    <?php if (get_post_meta(get_the_ID(), '_sgs_show_services', true) !== '0') : ?>
    ...
    <?php if (get_post_meta(get_the_ID(), '_sgs_show_reviews', true) !== '0') : ?>
      <section class="bg-white section-padding">
        <div class="container-custom">
          <div class="mb-12 text-center fade-in-up">
    ...
    <!-- Latest News Section -->
    <?php if (get_post_meta(get_the_ID(), '_sgs_show_news', true) !== '0') : ?>
      <section class="section-padding bg-neutral-50">
        <div class="container-custom">
          <div class="mb-12 text-center fade-in-up">
