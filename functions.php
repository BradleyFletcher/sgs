...
function sgs_theme_setup()
{
  ...
  register_nav_menus(array(
    ...
    'locations' => __('Locations Menu', 'sgs'),
...
function sgs_customize_register($wp_customize)
{
  ...
  // YouTube URL
  $wp_customize->add_setting('sgs_youtube_url', array(
    'default'           => '',
    'sanitize_callback' => 'esc_url_raw',
  ));
  $wp_customize->add_control('sgs_youtube_url', array(
    'label'    => __('YouTube URL', 'sgs'),
    'section'  => 'sgs_social_media',
    'type'     => 'url',
    'priority' => 45,
  ));
 ...
 */
function sgs_sanitize_whatsapp_number($number)
{
  if (empty($number)) {
    return '';
  }
  ...
  // Remove plus sign if present (wa.me works without it)
...
function sgs_social_icons($classes = '')
{
  ...
  $youtube = get_theme_mod('sgs_youtube_url');
  $whatsapp = get_theme_mod('sgs_whatsapp_number');

  if (!$facebook && !$twitter && !$instagram && !$tiktok && !$youtube && !$whatsapp) {
    return;
  }
  ...
  if ($youtube) : ?>
    <a href="<?php echo esc_url($youtube); ?>" target="_blank" rel="noopener noreferrer" class="transition-colors transform text-primary-900 hover:text-primary-700 hover:scale-110" aria-label="YouTube">
      <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
  ...
  if ($whatsapp) :
    $whatsapp_clean = sgs_sanitize_whatsapp_number($whatsapp);
  ?>
    <a href="https://wa.me/<?php echo esc_attr($whatsapp_clean); ?>" target="_blank" rel="noopener noreferrer" class="transition-colors transform text-primary-900 hover:text-primary-700 hover:scale-110" aria-label="WhatsApp">
      <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
...
function sgs_handle_contact_form()
{
  ...
  // Prepare email
  $to = array('info@superguttering.co.uk', 'bradhouston@gmail.com');
...
/**
 * Fix email sender and return-path to use proper domain
 */
function sgs_fix_email_from($email)
{
  return 'wordpress@superguttering.co.uk';
}
add_filter('wp_mail_from', 'sgs_fix_email_from');

function sgs_fix_email_from_name($name)
{
  return 'Super Guttering Services';
}
add_filter('wp_mail_from_name', 'sgs_fix_email_from_name');
...
function sgs_footer_social_icons()
{
  ...
  if ($youtube) : ?>
    <a href="<?php echo esc_url($youtube); ?>" target="_blank" rel="noopener noreferrer" class="flex justify-center items-center w-10 h-10 rounded-full transition-colors bg-primary-800 hover:bg-secondary-400 group" aria-label="YouTube">
      <svg class="w-5 h-5 text-white group-hover:text-primary-900" fill="currentColor" viewBox="0 0 24 24">
        <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
      </svg>
    </a>
  ...
  if ($whatsapp) :
    $whatsapp_clean = sgs_sanitize_whatsapp_number($whatsapp);
  ?>
    <a href="https://wa.me/<?php echo esc_attr($whatsapp_clean); ?>" target="_blank" rel="noopener noreferrer" class="flex justify-center items-center w-10 h-10 bg-green-500 rounded-full transition-colors hover:bg-green-600 group" aria-label="WhatsApp">
 ...
 */
require_once get_template_directory() . '/inc/homepage-meta-boxes.php';
